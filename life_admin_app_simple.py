#!/usr/bin/env python3
"""
Life Admin System - Mac Menu Bar App (Simplified)

A stable version that opens web UI for operations instead of running them directly.
This avoids threading issues and provides better visual feedback.
"""
...
class LifeAdminApp(rumps.App):
    ...
    def update_menu_titles(self):
        ...
        for key, item in self.menu.items():
            if hasattr(item, 'title') and item.title.startswith('Total Documents:'):
                item.title = f"Total Documents: {doc_count}"
                break
        ...
        for key, item in self.menu.items():
            if hasattr(item, 'title') and 'Generate All Summaries' in item.title:
                if summary_count > 0:
                    item.title = f"Generate All Summaries ({summary_count})"
                else:
                    item.title = "Generate All Summaries ✓"
                break
        ...
        for key, item in self.menu.items():
            if hasattr(item, 'title') and 'Generate Insights' in item.title:
                if insight_count > 0:
                    item.title = f"Generate Insights ({insight_count} active)"
                else:
                    item.title = "Generate Insights"
                break
    ...
    def generate_all_summaries(self, _):
        """Generate AI summaries - opens web UI."""
        if not self.check_server_running():
            rumps.alert("Server Not Running", "Please start the server first.")
            return
    ...
    def generate_insights(self, _):
        """Generate insights - opens dashboard."""
        if not self.check_server_running():
            rumps.alert("Server Not Running", "Please start the server first.")
            return
